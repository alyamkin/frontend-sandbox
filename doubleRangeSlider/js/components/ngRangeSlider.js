    ...
    function ngRangeSlider($window) {
      return {
        ...
        template: function template(element, attrs) {
          ...
          var inputElements =
            '<input list="numbers" type="range" ng-change="_which = 0" ng-model="_model[0]" min="{{_values.min}}" max="{{_values.max}}" step="{{_step}}" /><input type="range" ng-change="_which = 1" ng-model="_model[1]" min="{{_values.min}}" max="{{_values.max}}" step="{{_step}}" />';
          ...
          return (
            '<section><datalist id="numbers"><option ng-repeat="index in iter(max)">{{index}}</option></datalist>' +
            inputElements +
            '</section>'
          );
        },
        ...
        link: function link(scope, element) {
          ...
          var _reevaluateInputs = function _reevaluateInputs() {
            ...
            $angular.forEach(
              inputElements,
              function forEach(inputElement, index) {
          ...
          if (scope.throttle && scope._supportThrottle()) {
            ...
            _updateModel = _.throttle(
              _updateModel,
              $window.parseFloat(scope.throttle)
            );
          }
